...
This is a fork of the Lemonbar project.

Changes:

- With Xft support (from https://github.com/krypt-n/bar) 
- Fix Nerd Font icon width (from https://gitlab.com/protesilaos/lemonbar-xft)
- Synced with master branch (https://github.com/LemonBoy/bar)  Sep 02, 2024 
...
*lemonbar* \[-h \| -g *width***x***height***+***x***+***y* \| -O \| -b
\| -d \| -f *font* \| -p \| -n *name* \| -u *pixel* \| -B *color* \| -F
*color* \| -U *color* \| -o *offset*\]

# DESCRIPTION

**lemonbar** (formerly known as **bar**) is a lightweight statusbar
based on XCB. Provides full UTF-8 support, basic formatting, RandR and
Xinerama support and EWMH compliance without wasting your precious
memory.

# INPUT

The data to be parsed is read from the standard input, parsing and
printing the input data are delayed until a newline is found.
...
:   Set next output to *name*. May be used multiple times; order is
    significant. If any **-O** options are given, only **-O** specified
    monitors will be used. Invalid output names are silently ignored.
    (only supported on randr configurations at this time)
...
:   Define the font to load into one of the five slots (the number of
    ...
    Specifies a font to use. Can be used multiple times to load more
    than a single font.
...
:   Create a clickable area starting from the current position, when the
    area is clicked *command* is printed on stdout. The area is closed
    when a **A** token, not followed by : is encountered.
...
:   Change the monitor the bar is rendered to. *dir* can be either
    ...
    **nNAME**
    ...
    :   Named monitor. Eg. *%{SnHDMI-0} This text will show up on the
        HDMI-0 output*
